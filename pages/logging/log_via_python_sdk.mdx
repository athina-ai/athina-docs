...
```
- `cost (optional)`: is the cost incurred by the LLM for the given inference. Cost is also calculated internally(if not provided) only for supported language models.  
```python
cost: 0.01
```
