...
- `query`: Optional[str] - If passed this will be added to the payload.
- `context`: Optional[str] - If passed this will be added to the payload.
- `expected_response`: Optional[str] - If passed this will be added to the payload.
...
ApiCall(
    ...
    context="YC is a well-known startup accelerator based in Silicon Valley, California. Y Combinator is one of the most influential and successful startup accelerators globally. YC has invested in over 2,000 companies, including Dropbox, Airbnb, Stripe, Reddit, and Instacart.",
    expected_response="YC takes 7% equity."
).run_batch(data=dataset).to_df()
```
